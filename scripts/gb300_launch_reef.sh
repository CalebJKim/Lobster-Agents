...
# Launch the NemoClaw Reef backend and frontend on a local demo host.
...
MODEL_NAME="${NEMOCLAW_REEF_LLM_MODEL:-qwen3.6-27b-fp8}"
MAX_WORKERS="${NEMOCLAW_REEF_SANDBOX_MAX_CONCURRENT_OPENCLAW_RUNS:-2}"
...
if curl -fsS "http://127.0.0.1:${BACKEND_PORT}/health" >/dev/null 2>&1; then
  ...
  NEMOCLAW_REEF_LLM_BASE_URL="${NEMOCLAW_REEF_LLM_BASE_URL:-http://127.0.0.1:${VLLM_PORT}/v1}" \
  NEMOCLAW_REEF_LLM_MODEL="$MODEL_NAME" \
  NEMOCLAW_REEF_LLM_API_KEY="${NEMOCLAW_REEF_LLM_API_KEY:-dummy}" \
  NEMOCLAW_REEF_NEMOCLAW_PROVIDER="${NEMOCLAW_REEF_NEMOCLAW_PROVIDER:-custom}" \
  NEMOCLAW_REEF_NEMOCLAW_ENDPOINT_URL="${NEMOCLAW_REEF_NEMOCLAW_ENDPOINT_URL:-http://host.openshell.internal:${VLLM_PORT}/v1}" \
  NEMOCLAW_REEF_NEMOCLAW_MODEL="${NEMOCLAW_REEF_NEMOCLAW_MODEL:-$MODEL_NAME}" \
  NEMOCLAW_REEF_NEMOCLAW_API_KEY="${NEMOCLAW_REEF_NEMOCLAW_API_KEY:-dummy}" \
  NEMOCLAW_REEF_OPENCLAW_THINKING_LEVEL="${NEMOCLAW_REEF_OPENCLAW_THINKING_LEVEL:-off}" \
  NEMOCLAW_REEF_SANDBOX_MAX_CONCURRENT_OPENCLAW_RUNS="$MAX_WORKERS" \
  nohup python -m uvicorn --app-dir src nemoclaw_reef.main:app \
    --host 0.0.0.0 \
    --port "$BACKEND_PORT" \
    >"$BACKEND_LOG" 2>&1 &
