...
NEW_ENV_PREFIX = "NEMOCLAW_REEF_"
LEGACY_ENV_PREFIX = "OFFICE_AGENTS_"

for key, value in list(os.environ.items()):
    if not key.startswith(LEGACY_ENV_PREFIX):
        continue
    mapped_key = f"{NEW_ENV_PREFIX}{key[len(LEGACY_ENV_PREFIX):]}"
...
class Settings(BaseSettings):
    ...
    db_path: str = "nemoclaw_reef.db"
    allowed_file_paths: Annotated[list[str], NoDecode] = []
    tavily_api_key: str = ""

    # Path where the live-editable water-cooler topics file is found.
    # Override via NEMOCLAW_REEF_WATER_COOLER_TOPICS_PATH on demo hosts.
    water_cooler_topics_path: str = "examples/water-cooler-topics.md"

    # Reef autonomy controls whether idle agents chat when no user query or sandbox run is active.
    ...
    # Per-call timeout for reef chat. Local demo models can warm up slowly on first call.
    ...
    # Some local vLLM routes do not support model-facing tool schemas.
    ...
    # Extra directories searched by integrations._which() when nemoclaw /
