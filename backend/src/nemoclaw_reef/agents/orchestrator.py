...
from nemoclaw_reef.models import Action, ActionType, AgentState, ReefEvent
...
from nemoclaw_reef.reef import IdleChat, QueryIntake
from nemoclaw_reef.services.sandbox_manager import SandboxManager, short_sandbox_name
...
class Orchestrator:
    ...
    def __init__(
        ...
        self.water_cooler_enabled = settings.autonomy_enabled  # runtime toggle for idle reef chat
    ...
    async def submit_query(
        ...
        """User submits a new question for the free reef agents to work on."""
    ...
    async def add_lobster(self, agent: Agent) -> Agent:
        ...
        # Register with reef state so the frontend sees them on the next
        # /state poll + full_state broadcast.
        from nemoclaw_reef.claw_config import get_claw_metadata
        self.reef_state.register_agent(
            name=agent.name,
            role=agent.role,
    ...
    async def remove_lobster(self, name: str) -> bool:
        ...
        self.reef_state.agent_states.pop(name, None)
        await self.broadcast({
    ...
    async def run(self) -> None:
        ...
        while self.running:
            ...
            if self.reef_state.current_query:
    ...
    async def _process_batch_outcomes(
        ...
        for outcome in outcomes:
            ...
            self.reef_state.update_from_action(
                agent.name,
                action,
            ...
            if action.type == ActionType.write_whiteboard and self.reef_state.current_query:
    ...
    async def _finalize_query(self) -> None:
        ...
        """
        query_text = self.reef_state.current_query or ""
        self._finish_general_query_state()
        if self.reef_state.speech_language == "zh":
    ...
    async def _tick(self) -> None:
        ...
        if not self.reef_state.current_query:
        ...
        for batch in self._group_into_batches(active_agents):
            ...
            if not self.reef_state.current_query and not self.query_queue.empty():
    ...
    async def _inject_convergence_pressure(self) -> None:
        ...
        if self._query_tick >= QUERY_TIMEOUT_TICK:
            ...
            query_text = self.reef_state.current_query
            self._finish_general_query_state()
            await self.broadcast({
                "type": "query_completed",
                "query": query_text,
    ...
    def _action_to_event(
        ...
        """Convert an agent action + result into an ReefEvent for others.
        ...
        await self.broadcast(
            {
                "type": "full_state",
                "agents": agents_info,
                "office": self.reef_state.to_dict(),
