...
from nemoclaw_reef.services.sandbox_manager import short_sandbox_name
...
def load_topics() -> list[str]:
    ...
    override via the ``NEMOCLAW_REEF_WATER_COOLER_TOPICS_PATH`` env var.
    """
...
class IdleChat:
    ...
    async def tick(
        ...
        *,
        ...
        if self._reef_state.current_query:
            return
        if not enabled:
            return
        ...
        if llm_error is not None:
            from nemoclaw_reef.config import settings as _settings
    ...
    def _build_prompt(
        ...
        *,
        ...
        return (
            ...
            f"{LANGUAGE_INSTRUCTIONS.get(self._reef_state.speech_language, LANGUAGE_INSTRUCTIONS['en'])}\n"
            f"{history_block}"
            f"{last_line_callout}"
            f"{direction}"
    ...
    async def _call_llm_with_abort(
        ...
        from nemoclaw_reef.config import settings as _settings  # local import keeps module-load order simple
        ...
        system_prompt = (
            ...
            + LANGUAGE_INSTRUCTIONS.get(
                self._reef_state.speech_language,
