...
from nemoclaw_reef.integrations.openclaw_cli import (
    DEFAULT_RUNS_WORKDIR,
    ensure_openclaw_agent,
    run_openclaw,
)
from nemoclaw_reef.integrations.hermes_runner import run_hermes
from nemoclaw_reef.integrations.nemoclaw_cli import get_policy_presets
...
class SandboxManager:
    ...
    def _place_agent(self, agent: Agent, sandbox_name: str) -> None:
        ...
        self._reef_state.update_agent_position(agent.name, target_room, agent.position)
    ...
    def _release_agent(self, agent: Agent) -> None:
        next_room = "war_room" if self._reef_state.current_query else "break_room"
        release_room_seat(agent.location, agent.name)
        ...
        agent.target_position = None
        agent.current_task = self._reef_state.current_query
        agent.state = AgentState.collaborating if self._reef_state.current_query else AgentState.idle
        self._reef_state.update_agent_position(agent.name, next_room, agent.position)
        state = self._reef_state.agent_states.setdefault(agent.name, {})
    ...
    async def _run_one_agent(
        ...
        result = await run_openclaw(
            ...
            speech_language=self._reef_state.speech_language,
