...
from office_agents.claw_config import CLAW_IDENTITIES, SANDBOX_WORKSPACES, SandboxWorkspace
...
from office_agents.office.layout import ROOM_POSITIONS, ROOMS, get_room_position, release_room_seat
...
async def cleanup_demo(req: dict | None = None) -> dict[str, object]:
    ...
    await _reset_reef_state(orch, office)
...
async def _reset_reef_state(orch, office) -> None:
    ...
    for agent in orch.agents:
        ...
        state = office.agent_states.setdefault(agent.name, {})
        state["state"] = "idle"
